...
Same pattern as subtitles: the UIKit path inserts it into `containerView` between `subtitleView` and `controlsView`; the SwiftUI path wraps it in a representable inside the ZStack. Rendering, layout, and animation logic live in one file; the UI-path code is thin wiring only. The overlay is owned by the presentation views (which read the config), not by `TPAVPlayer`. Positioning against view bounds (decision 4) removes any dependency on `AVPlayerLayer.videoRect` or the private `playerLayer` property.
