...
- [ ] 2.5 Add KVO observation of `TPStreamPlayer.currentTime` to look up active cue and update the label text
- [ ] 2.6 Properly remove KVO observer in `deinit` / `willMove(toSuperview: nil)`
- [ ] 2.7 Show empty state while WebVTT is downloading
- [ ] 2.8 Handle download failures gracefully (hide subtitle view)

## 3. UIKit Integration

- [ ] 3.1 Add `startWithCaption: Bool` to `TPStreamPlayerConfiguration` and builder method
...
- [ ] 3.8 Implement auto-selection on player "ready": if `startWithCaption == true`, auto-set the first available subtitle track
- [ ] 3.9 Update `showSettingsButton` to include `enableCaptions`

## 4. SwiftUI Integration

- [ ] 4.1 Create SwiftUI subtitle view in `Source/Views/SwiftUI/` with a `Text` view and observe `player.observedCurrentTime` for cue lookup
